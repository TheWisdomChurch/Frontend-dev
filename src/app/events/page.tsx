...
export default function EventsPage() {
  ...
  const flatEvents: EventItem[] = useMemo(() => {
    return calendarGrid
      .flatMap(day => day.events.map(evt => ({
        title: evt.title,
  ...
  const featured = filteredEvents[0] ?? flatEvents[0];
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition ${
                    filter === btn.key ? 'bg-white text-black border-white' : 'bg-white/5 text-white border-white/10'
                  }`}
                >
                  {btn.label}
                </button>
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 via-black/60 to-black/80 p-6 sm:p-7 lg:p-8 shadow-[0_18px_50px_rgba(0,0,0,0.45)] space-y-4">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-white/70">
                <span className="inline-flex items-center gap-2">
                  <Calendar className="w-4 h-4" style={{ color: colorScheme.primary }} />
